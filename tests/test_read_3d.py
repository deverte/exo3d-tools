...
def test_read_3d_dat(tmp_path, text_data_3d, file_generator):
    ...
    assert (data.grid.v[0] == np.meshgrid(theta_v, phi_v, r_v)[0]).all()
    assert (data.grid.v[1] == np.meshgrid(theta_v, phi_v, r_v)[1]).all()
    assert (data.grid.v[2] == np.meshgrid(theta_v, phi_v, r_v)[2]).all()
    assert (data.grid.n[0] == np.meshgrid(theta_n, phi_n, r_n)[0]).all()
    assert (data.grid.n[1] == np.meshgrid(theta_n, phi_n, r_n)[1]).all()
    assert (data.grid.n[2] == np.meshgrid(theta_n, phi_n, r_n)[2]).all()